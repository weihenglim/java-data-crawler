...
public class Analyzer {
    ...
    public static String analyze(Date start, Date end, String path) {
        return analyze("bitcoin", start, end, path);
    }
    
    /* The method itself */
    public static String analyze(String keyword, Date start, Date end, String upath) {
        ...
        RedditCrawler reddit = new RedditCrawler("BitcoinMarkets", "Daily Discussion", "top", upath);
        TwitterCrawler twitter = new TwitterCrawler();
        twitter.TwitterCrawlerP(upath);

        /* Retrieve prices from Bitmex */
        Bitmex bitmex = new Bitmex().startDate(start).endDate(end);
        bitmex.BitPath(upath);
        ...
        String path = String.format("%s\\analysis-%s.txt", upath, end.toString());
